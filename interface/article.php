...
class mainpage extends connector {
	...
	function in_list() {
		...
		if ($styleid != 4) {
			if (!empty($typeread['headtitle'])) {
				$this->lng['sitename'] = $typeread['headtitle'] . $this->lng['sitename'];
			}
		...
		if ($styleid == 1) {
			...
			if ($typeread['linkid']) {
				...
				if (!empty($readinfo['headtitle'])) {
					$this->lng['sitename'] = $readinfo['headtitle'] . $this->lng['sitename'];
				}
	...
	function in_read() {
		...
		$read_templates = admin_WAP ? $typeread['wapreadtemplate'] : ($readinfo['istemplates'] && !empty($readinfo['template'])) ? $readinfo['template'] : $typeview['readtemplate'];
