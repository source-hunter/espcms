...
define('DISCUZ_ROOT2', substr(dirname(__FILE__), 0, -3));
define('DISCUZ_ROOT', DISCUZ_ROOT2 . 'public/');
include DISCUZ_ROOT2 . 'datacache/command.php';

if (!defined('IN_UC')) {

	set_magic_quotes_runtime(0);
	defined('MAGIC_QUOTES_GPC') || define('MAGIC_QUOTES_GPC', get_magic_quotes_gpc());
	include_once DISCUZ_ROOT . './uc_config.php';
...
class uc_note {
	...
	function uc_note() {
		...
		$this->dbconfig = $this->appdir . './public.php';
		$this->db = $GLOBALS['db'];
	...
	function deleteuser($get, $post) {
		...
		$query = $this->db->query("DELETE FROM " . $this->tablepre . "member WHERE id IN ($uids)");
	...
	function gettag($get, $post) {
		...
		while ($tagthread = $this->db->fetch_array($query)) {
			if ($tagthread['tid']) {
				$threadlist[] = array('subject' => $tagthread['subject'], 'uid' => $tagthread['authorid'], 'username' => $tagthread['author'], 'dateline' => $tagthread['dateline'], 'url' => $boardurl . 'viewthread.php?tid=' . $tagthread['tid']);
			}
		}
	...
	function updateapps($get, $post) {
		...
		if (is_writeable($this->appdir . './uc_config.php')) {
			$configfile = trim(file_get_contents($this->appdir . './uc_config.php'));
			$configfile = substr($configfile, -2) == '?>' ? substr($configfile, 0, -2) : $configfile;
			$configfile = preg_replace("/define\('UC_API',\s*'.*?'\);/i", "define('UC_API', '$UC_API');", $configfile);
			if ($fp = @fopen($this->appdir . './uc_config.php', 'w')) {
			...
			foreach ($credit as $appid => $credititems) {
				if ($appid == UC_APPID) {
					foreach ($credititems as $value) {
						$outextcredits[] = array('appiddesc' => $value['appiddesc'], 'creditdesc' => $value['creditdesc'], 'creditsrc' => $value['creditsrc'], 'title' => $value['title'], 'unit' => $value['unit'], 'ratiosrc' => $value['ratiosrc'], 'ratiodesc' => $value['ratiodesc'], 'ratio' => $value['ratio']);
					}
				}
			}
		}
